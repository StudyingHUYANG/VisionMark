...
router.get('/overview', (req, res) => {
  try {
    ...
    // 查询总标注数（segments表）
    const totalAnnotations = db.prepare('SELECT COUNT(*) as count FROM segments').get();
    // 总投票数（暂为0，投票系统完成后补充）
    const totalVotes = 0;
...
router.get('/user/contributions', (req, res) => {
  ...
  try {
    ...
    const contributions = db.prepare(`
      SELECT 
        s.id, v.bvid, s.start_time, s.end_time, s.type, s.contributor_id
      FROM segments s
    ...
    const total = db.prepare(`
      SELECT COUNT(*) as count FROM segments WHERE contributor_id = ?
    `).get(user_id);
...
router.get('/popular-videos', (req, res) => {
  try {
    // 查询标注最多的视频TOP20（关联segments和videos表）
